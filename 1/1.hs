...
-- more efficient answer. solve1 and solve2 work for the challenge, but this is makign those answers better
pairs :: [a] -> [(a, a)]
pairs [] = []
pairs (x:xs) = [(x, y) | y <- xs] ++ pairs xs
 
solve1' :: [Int] -> Int
solve1' = head . map (\(a,b) -> a*b) . filter (\(a,b) -> a+b == 2020) . pairs
